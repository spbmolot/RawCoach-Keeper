@php
    $storageKey = 'shopping_' . ($startDate?->format('Y-m-d') ?? 'default') . '_' . ($endDate?->format('Y-m-d') ?? 'default');
    $allItems = collect();
    $flatIndex = 0;
    $itemsJson = [];
    if (isset($shoppingList)) {
        foreach ($shoppingList as $category => $items) {
            foreach ($items as $item) {
                $name = $item['ingredient']->ingredient_name ?? ($item['name'] ?? '');
                $amount = round($item['amount'], 1);
                $unit = $item['unit'] ?? '';
                $itemsJson[] = ['name' => $name, 'amount' => $amount, 'unit' => $unit, 'category' => $category];
                $flatIndex++;
            }
        }
    }
...
<x-app-layout>
    <div class="max-w-4xl mx-auto" x-data="shoppingList({{ json_encode($itemsJson) }}, '{{ $storageKey }}')">
        {{-- Заголовок --}}
        ...
        @if($totalItems > 0)
            {{-- Прогресс --}}
            <div class="bg-white rounded-xl sm:rounded-2xl shadow-sm border border-gray-100 p-4 sm:p-5 mb-4 sm:mb-6">
                <div class="flex items-center justify-between mb-2 sm:mb-3">
                    ...
                    <div class="flex items-center gap-2">
                        ...
                        <button x-show="checkedCount > 0" x-transition @click="clearAll()" class="text-xs text-gray-400 hover:text-red-500 underline underline-offset-2 ml-2 transition">
                            сбросить
                        </button>
                    </div>
                </div>
            ...
            <div x-show="allChecked" x-transition.duration.500ms class="bg-gradient-to-r from-green-500 to-emerald-600 rounded-xl sm:rounded-2xl shadow-sm p-5 sm:p-6 mb-4 sm:mb-6 text-center text-white" x-cloak>
            ...
            @foreach($shoppingList as $category => $items)
                <div class="bg-white rounded-xl sm:rounded-2xl shadow-sm border border-gray-100 overflow-hidden mb-4 sm:mb-6">
                        ...
                        <div class="flex items-center justify-between">
                            <div class="flex items-center gap-2 sm:gap-3">
                                <div class="w-8 h-8 sm:w-10 sm:h-10 bg-green-100 rounded-lg sm:rounded-xl flex items-center justify-center">
                                    <i data-lucide="tag" class="w-4 h-4 sm:w-5 sm:h-5 text-green-600"></i>
                                </div>
                                <div>
                                    <h3 class="font-semibold text-gray-900 text-sm sm:text-base">
                                        @switch($category)
                                            @case('fruits') Фрукты @break
                                            @case('vegetables') Овощи @break
                                            @case('dairy') Молочные продукты @break
                                            @case('meat') Мясо @break
                                            @case('fish') Рыба @break
                                            @case('grains') Крупы и злаки @break
                                            @case('spices') Специи @break
                                            @case('oils') Масла @break
                                            @case('nuts') Орехи и семена @break
                                            @case('other') Прочее @break
                                            @default {{ $category }}
                                        @endswitch
                                    </h3>
                                    <p class="text-xs sm:text-sm text-gray-500">{{ count($items) }} {{ count($items) == 1 ? 'продукт' : (count($items) < 5 ? 'продукта' : 'продуктов') }}</p>
                                </div>
                            </div>
                        </div>
                    </div>
                    <div class="divide-y divide-gray-100">
                        @foreach($items as $item)
                            @php
                                $itemName = $item['ingredient']->ingredient_name ?? ($item['name'] ?? '');
                                $itemAmount = round($item['amount'], 1);
                                $itemUnit = $item['unit'] ?? '';
                            @endphp
                            <label class="flex items-center gap-3 sm:gap-4 p-3 sm:p-4 hover:bg-gray-50 transition cursor-pointer group" :class="checked[{{ $idx }}] ? 'bg-green-50/50' : ''">
                                <input 
                                    type="checkbox" 
                                    ...
                                    @change="toggle({{ $idx }})"
                                >
                                <span class="flex-1 text-sm sm:text-base transition-all duration-200" :class="checked[{{ $idx }}] ? 'text-gray-400 line-through' : 'text-gray-900'">{{ $itemName }}</span>
                                <span class="font-medium text-xs sm:text-sm flex-shrink-0 transition-all duration-200" :class="checked[{{ $idx }}] ? 'text-gray-300' : 'text-gray-500'">{{ $itemAmount }} {{ $itemUnit }}</span>
                            </label>
                            @php $idx++; @endphp
                        @endforeach
                    </div>
    ...
    @push('scripts')
    <script>
        function shoppingList(items, storageKey) {
            return {
                items: items,
