...
@section('title', "Поиск: {$query} — Рецепты — RawPlan")
@section('description', "Результаты поиска рецептов по запросу «{$query}»")
...
@section('content')
    ...
    <section class="hero-gradient py-10 sm:py-16">
        <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 text-center text-white">
            <h1 class="text-3xl sm:text-4xl md:text-5xl font-bold mb-2 sm:mb-4">Поиск рецептов</h1>
            <p class="text-base sm:text-xl text-green-100 max-w-2xl mx-auto">Результаты по запросу «{{ $query }}»</p>
        </div>
    </section>
    ...
    <div class="bg-white border-b border-gray-100 sticky top-16 z-40">
        <div class="max-w-7xl mx-auto px-3 sm:px-6 lg:px-8 py-3 sm:py-4">
            <form method="GET" action="{{ route('recipes.search') }}" class="flex gap-3">
                <div class="flex-1 relative">
                    <i data-lucide="search" class="w-4 h-4 text-gray-400 absolute left-3 top-1/2 -translate-y-1/2"></i>
                    <input type="text" name="q" value="{{ $query }}" placeholder="Поиск по названию, ингредиентам..." class="w-full pl-10 rounded-full border-gray-300 focus:border-green-500 focus:ring-green-500 text-sm" required minlength="2">
                </div>
                <button type="submit" class="px-5 py-2.5 bg-green-500 hover:bg-green-600 text-white rounded-full text-sm font-medium transition">
                    Найти
                </button>
            </form>
        </div>
