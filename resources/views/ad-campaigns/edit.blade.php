<x-app-layout>
    <div class="max-w-3xl mx-auto">
        ...
        <div class="bg-white rounded-2xl shadow-sm border border-gray-100 p-6 sm:p-8">
            <div class="mb-6">
                <h1 class="text-2xl font-bold text-gray-900">Редактирование кампании</h1>
                <p class="text-gray-600 mt-1 text-sm">{{ $adCampaign->name }}</p>
            </div>
            ...
            <form action="{{ route('ad-campaigns.update', $adCampaign) }}" method="POST" class="space-y-6">
                @csrf
                @method('PUT')

                <div>
                    <label for="name" class="block text-sm font-medium text-gray-700 mb-1">Название кампании <span class="text-red-500">*</span></label>
                    <input type="text" name="name" id="name" value="{{ old('name', $adCampaign->name) }}" maxlength="255" class="w-full rounded-xl border-gray-300 focus:border-green-500 focus:ring-green-500 text-sm" required>
                    @error('name') <p class="mt-1 text-sm text-red-600">{{ $message }}</p> @enderror
                </div>

                <div>
                    <label for="description" class="block text-sm font-medium text-gray-700 mb-1">Описание</label>
                    <textarea name="description" id="description" rows="3" maxlength="1000" class="w-full rounded-xl border-gray-300 focus:border-green-500 focus:ring-green-500 text-sm">{{ old('description', $adCampaign->description) }}</textarea>
                ...
                <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
                    <div>
                        ...
                        <input type="number" name="budget" id="budget" value="{{ old('budget', $adCampaign->budget) }}" min="0" step="0.01" class="w-full rounded-xl border-gray-300 focus:border-green-500 focus:ring-green-500 text-sm" required>
                    ...
                    <div>
                        ...
                        <input type="number" name="daily_budget" id="daily_budget" value="{{ old('daily_budget', $adCampaign->daily_budget) }}" min="0" step="0.01" class="w-full rounded-xl border-gray-300 focus:border-green-500 focus:ring-green-500 text-sm">
                ...
                <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
                    <div>
                        ...
                        <input type="date" name="starts_at" id="starts_at" value="{{ old('starts_at', \Carbon\Carbon::parse($adCampaign->starts_at)->format('Y-m-d')) }}" class="w-full rounded-xl border-gray-300 focus:border-green-500 focus:ring-green-500 text-sm" required>
                    ...
                    <div>
                        ...
                        <input type="date" name="ends_at" id="ends_at" value="{{ old('ends_at', \Carbon\Carbon::parse($adCampaign->ends_at)->format('Y-m-d')) }}" class="w-full rounded-xl border-gray-300 focus:border-green-500 focus:ring-green-500 text-sm" required>
                ...
                @if($placements->isNotEmpty())
                    <div>
                        ...
                        @php $currentPlacements = old('placement_ids', $adCampaign->placements->pluck('id')->toArray()); @endphp
                        <div class="space-y-2">
                            @foreach($placements as $placement)
                                <label class="flex items-center gap-3 p-3 border rounded-xl cursor-pointer hover:border-green-300 transition has-[:checked]:border-green-500 has-[:checked]:bg-green-50">
                                    <input type="checkbox" name="placement_ids[]" value="{{ $placement->id }}" {{ in_array($placement->id, $currentPlacements) ? 'checked' : '' }} class="rounded text-green-500 focus:ring-green-500">
                                    <div>
                ...
                <div>
                    ...
                    <textarea name="target_audience" id="target_audience" rows="3" class="w-full rounded-xl border-gray-300 focus:border-green-500 focus:ring-green-500 text-sm font-mono text-xs">{{ old('target_audience', $adCampaign->target_audience ? json_encode($adCampaign->target_audience, JSON_PRETTY_PRINT | JSON_UNESCAPED_UNICODE) : '') }}</textarea>
                ...
                <div class="pt-4 border-t border-gray-100 flex flex-col sm:flex-row gap-3">
                    ...
                    <a href="{{ route('ad-campaigns.show', $adCampaign) }}" class="inline-flex items-center justify-center gap-2 px-6 py-3 bg-gray-100 hover:bg-gray-200 text-gray-700 rounded-xl font-medium transition">
                        Отмена
                    </a>
                </div>
            </form>
